...
RUN cp instance/config.example.py instance/config.py
    ...
    # sed -i "s|^MAX_CONTENT_LENGTH = .*|MAX_CONTENT_LENGTH = 3 \* 1024 \* 1024 \* 1024|" "instance/config.py" && \
    # sed -i "s|^FHOST_MIN_EXPIRATION = .*|FHOST_MIN_EXPIRATION = 3 \* 24 \* 60 \* 60 \* 1000|" "instance/config.py" && \
    # sed -i "s|^FHOST_MAX_EXPIRATION = .*|FHOST_MAX_EXPIRATION = 30  \* 24 \* 60 \* 60 \* 1000|" "instance/config.py" && \
...
CMD [ "/bin/bash", "-c", "while true; do FLASK_APP=fhost flask prune && sleep 300; done & /bin/bash entrypoint.sh" ]
